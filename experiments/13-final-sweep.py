"""
Final sweep:
  datasets × encoders × detectors × metrics × window_sizes

Grid:
  datasets     : DATASETS list (configurable)
  encoders     : ModernBERT, Jina-v2
  detectors    : DDM, EDDM, ADWIN
  metrics      : MMD, JSD  (which signal feeds the detector)
  window_sizes : [50, 100, 150, 200, 250]  (N_REF_EMB = WINDOW_SIZE)

Detection logic:
  - warning → start filling warning buffer with incoming windows
  - drift   → retrain on buffer (+ current window), reset buffer and detector

Outputs (per run):
  results/per_window/{tag}.csv   ← full per-window detail
  results/plots/{tag}.png        ← metrics timeline plot
  results/summary.csv            ← one row per completed run (appended)
"""
...
# ── Output dirs ────────────────────────────────────────────────────────────────
RESULTS_DIR    = "final_sweep_results"
...
# rolling window for prequential accuracy display
# updated dynamically to 4 × WINDOW_SIZE inside the run
ACC_PLOT_WINDOW_FACTOR = 4

TRAIN_EPOCHS = 3
TRAIN_BATCH  = 16

# Thresholds used by binary detectors (DDM/EDDM) to binarise metric values
MMD_THRESHOLD = 0.20
JSD_THRESHOLD = 0.10
...
ADAPT_MIN_SAMPLES   = 8
WARNING_BUFFER_MAX  = 500      # max items kept in warning buffer

SAVE_SCATTER_PLOTS  = False    # t-SNE scatter per drift event (slow for large sweeps)

# ── Sweep grid ─────────────────────────────────────────────────────────────────
SWEEP_WINDOW_SIZES = [50, 100, 150, 200, 250]
...
def save_results_plot(
    ...
    run_tag, base, subset, encoder_name, detector_name, metric_name, window_size,
):
    ...
    fig.suptitle(
        ...
        f"det={detector_name.upper()}  metric={metric_name.upper()}  "
        f"W={window_size}",
        fontsize=12,
    )
...
def run_experiment(
    ...
    """Run one experiment configuration and return summary dict."""
    ...
    for i in range(n_windows):
        ...
        # feed the chosen metric to the detector
        ...
        if warned:
            ...
            # accumulate buffer (cap to avoid unbounded growth)
        ...
        if drifted:
            ...
            # build adaptation set from warning buffer + current window
            adapt_X = np.array(warn_buffer_X + X_win.tolist())
            adapt_y = np.array(warn_buffer_y + y_win.tolist())

            if SAVE_SCATTER_PLOTS or True:
                # select samples via convex hull
                from sklearn.decomposition import PCA
                pca_coords  = PCA(n_components=2, random_state=42).fit_transform(win_np)
                X_sel, y_sel, _ = select_drift_samples(
                    ref_np, win_np, X_win, y_win, pca_coords
                )
            else:
                X_sel, y_sel = X_win, y_win
...
total_runs = (len(DATASETS) * len(ENCODERS) *
              len(SWEEP_WINDOW_SIZES) * len(SWEEP_DETECTORS) * len(SWEEP_METRICS))
print(f"\nFinal sweep: {total_runs} total runs")
print(f"  datasets={len(DATASETS)}  encoders={len(ENCODERS)}  "
      f"window_sizes={SWEEP_WINDOW_SIZES}  "
      f"detectors={SWEEP_DETECTORS}  metrics={SWEEP_METRICS}\n")
...
for base, subset in DATASETS:
    ...
    for enc in ENCODERS:
        ...
        for window_size in SWEEP_WINDOW_SIZES:
            n_ref_emb = window_size   # always match

            # ── train burn-in once per (encoder × window_size) ─────────────
            ...
            # snapshot state → reused for each (detector, metric) combo
            initial_state = copy.deepcopy(model.state_dict())

            for detector_name in SWEEP_DETECTORS:
                for metric_name in SWEEP_METRICS:
                    run_tag = (f"{base}{subset}_{enc['name'].lower()}_"
                               f"{detector_name}_{metric_name}_w{window_size}")
                    ...
                    # restore model to post-burnin state
