"""
Experiment 17 — Full Grid Sweep: Baseline vs Retrain

Datasets  : 5 Airbnb (comdrift-1..5-1-ss) + tech_non_tech
Encoders  : ModernBERT, Jina-v2
Selection : convex_hull, convex_hull_per_class
Window    : 50, 100, 200
Detectors : ADWIN, Rolling Baseline, Page-Hinkley   (all label-free, MMD/JSD-based)
Metrics   : MMD, JSD

Total retrain runs : 6 × 2 × 3 × 2 × 3 × 2 = 432
Baseline runs      : 6 × 2 × 3              =  36
                                              ────
Total passes       :                          468

Outputs:
  experiment17_results/plots/{tag}.png
  experiment17_results/per_window/{tag}.csv
  experiment17_results/summary.csv
"""
...
# ── Output dirs ────────────────────────────────────────────────────────────────
RESULTS_DIR = "experiment20_final_results"
...
SUMMARY_FIELDS = [
    ...
    "mean_macro_f1_baseline", "mean_macro_f1_retrain",
    "n_adaptations", "wall_time_s", "peak_gpu_mem_mb",
]

PER_WINDOW_FIELDS = [
    "window_idx", "window_pos",
    "prequential_acc", "macro_f1", "mmd", "jsd",
...
class RollingBaselineDetector:
    """Estimates mean/std of the signal during warmup, then flags deviations."""
...
def pass_baseline(model, tokenizer, inv_label_map,
    ...
    for i in range(n_windows):
        ...
        per_window_rows.append({
            ...
            "drift":           0,
            "adapted":         0,
            "wall_time_s":     round(wall_t, 4),
            "gpu_mem_mb":      round(gpu_mem, 1),
        })

    return positions, accuracies, per_window_rows
...
def pass_retrain(model, tokenizer, label_map, inv_label_map,
    ...
    for i in range(n_windows):
        ...
        if drifted:
            ...
            if pct_labeled < 1.0 and len(X_new) > 1:
                n_keep = max(1, int(round(len(X_new) * pct_labeled)))
                keep_idx = np.random.choice(len(X_new), n_keep, replace=False)
                X_new = X_new[keep_idx]
                y_new = y_new[keep_idx]
...
total_combos = (len(DATASETS) * len(ENCODERS) * len(SWEEP_SEEDS)
                * len(SWEEP_WINDOW_SIZES) * len(SWEEP_SELECTION_METHODS)
                * len(SWEEP_METRICS) * len(DETECTOR_FACTORIES)
                * len(SWEEP_PCT_LABELED))
print(f"\nExperiment 17 — Full Grid Sweep")
print(f"  {len(DATASETS)} datasets × {len(ENCODERS)} encoders"
      f" × {len(SWEEP_SEEDS)} seeds"
      ...
      f" × {len(SWEEP_PCT_LABELED)} pct_labeled"
...
def _pct_tag(pct: float) -> str:
    return f"{int(round(pct * 100)):03d}"
    ...
    for enc in ENCODERS:
        ...
        for seed in SWEEP_SEEDS:
            ...
            for window_size in SWEEP_WINDOW_SIZES:
                ...
                # ── baseline (once per window_size × seed) ──────────────
                base_tag  = f"{enc_tag.lower()}_baseline_w{window_size}_s{seed}"
                ...
                for sel_method in SWEEP_SELECTION_METHODS:
                    for metric_name in SWEEP_METRICS:
                        for pct_labeled in SWEEP_PCT_LABELED:
                            pct_tag = _pct_tag(pct_labeled)
                            ...
                            for det_name, det_factory in DETECTOR_FACTORIES.items():
                                ...
                                macro_f1_r = [r["macro_f1"] for r in pw_rows]
                                peak_mem = max((r["gpu_mem_mb"] for r in pw_rows),
                                               default=0.0)
                                ...
                                _append_summary({
                                    ...
                                    "mean_macro_f1_baseline":
                                        round(float(np.mean(base_macro_f1)), 4)
                                        if len(base_macro_f1) else "n/a",
                                    "mean_macro_f1_retrain":
                                        round(float(np.mean(macro_f1_r)), 4)
                                        if macro_f1_r else "n/a",
                            ...
                            if not os.path.exists(plot_path) and retrain_results:
                                save_plot(
                                    ...
                                    retrain_results,
                                    plot_tag,
                                    f"{dataset_name}  {enc_tag}  pct={pct_labeled}  seed={seed}",
                                    enc_tag,
            ...
            print(f"\n  [ENC/SEED DONE] {enc_tag} seed={seed}  total={wall_enc}s")
...
print(f"\n{'='*70}")
print(f"Experiment 20 complete.  {completed} runs.")
