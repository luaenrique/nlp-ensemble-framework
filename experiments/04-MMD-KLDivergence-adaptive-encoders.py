...
ENCODERS = [
    {
        ...
        "out_path"          : "exp_04_drift_modernbert_ss_2.png",
    },
    {
        "name"              : "Jina-v2",
        ...
        "out_path"          : "exp_04drift_jina_ss_2.png",
    },
]
...
def plot_results(
    ...
    mmd_scores, kl_scores, centroid_scores,
    ref_acc, ref_entropy,
    ...
    ax5 = axes[4]
    ax5.plot(window_positions, centroid_scores, color="mediumpurple", linewidth=1.3, label="Centroid distance")
    ax5.fill_between(window_positions, centroid_scores, alpha=0.15, color="mediumpurple")
    ax5.set_ylabel("Centroid distance", fontsize=11)
    ax5.set_xlabel("Position in comdrift stream (sample index)", fontsize=11)
    ...
    ax5.set_xticklabels([f"{x // 1_000}k" for x in x_ticks])
    ax5.set_xlim(0, N_DRIFTED)
    axes[4].legend(handles=legend_patches, loc="upper right", fontsize=9)
...
for enc in ENCODERS:
    ...
    for i in range(n_windows):
        ...
        print(f"  [{enc['name']}] Window {i+1}/{n_windows}  acc={acc:.3f}  entropy={entropy:.4f}  mmd={score:.4f}  kl={kl:.4f}  centroid={centroid_dist:.4f}")
